...
def map_reply(msg):
    ...
    if msg['Type'] == 'Picture':
        # msg['Text'](msg['FileName'])
        itchat.send_msg("[Master陈不在.暂由助理Neo回复你.有事请留言.主人看到后会及时回复你.] ", userId)
        itchat.send_image(msg['FileName'],userId)
        # itchat.send_msg('上图为微软小冰回答', userId)
    else:
        itchat.send_msg("[Master陈不在.暂由助理Neo回复你.有事请留言.主人看到后会及时回复你.] "+ text, userId)
...
# 向智能小冰提问
def xbAnswer(msg):
    xb = itchat.search_mps(name='Dercy')[0]
    quest = getText(msg)
    print(msg['FileName'])
    print(quest)
    if msg['Type'] == 'Picture':
